...
* Drop your own DB, setup a database for them, and run migrations:
...
Observe that the most popular browsers are:
...
Observe that the most popluar Operating Systems are:
...
1. `/galleries`
2. `/galleries/id` where `id` is 1, 2, or 17
3. `/galleries/id` where `id` is 1, 2, or 17
