...
In this project you'll use Ruby to build a graphical flashcard learning system.
...
You are to build a playable flashcard quizzing game fronted by a pre-developed
graphical user interface. Your job is to build the business logic that makes
it all work.

### Starter Code

You should begin by cloning the starter repo at http://github.com/turingschool-examples/flashback

### Run It

With just the starter repo you can boot up the game:

```
ruby flashback.rb
```

After a few second you should see the game interface.

### Playing a Game

* First, the player selects a dataset
* The program will then fetch the dataset from the server
* The game begins
* The user has sixty seconds to get as many answers correct as they can
* They are then presented with their results/statistics and can choose to play again

## Base Expectations

Your goal is to fill in the pieces of logic to make the game really work. These
can be grouped into three areas:

### Data Retrieval & Manipulation

We want the game data to be flexible. We'll accomplish that by hosting the data
on a web server and your game will pull down that data on the fly. That way we
can continue to grow and change the data without changing the code.
...
* The player starts the game by running `ruby flashback.rb`
* Then they see:
