...
* Concept:
   ...
   -  Core: Update gem to access google calendar. Fetch google calendar availability for you and your contacts. Scrape Adventures for scheduling in your preferred City.  
      compare availability and suggest adventures for you and your group to go on together.
...
* Concept:  Date night.  
* Problem:  LS is all about curating experience, but T&D more transactional.  Seamless can deliver food.  We need to make ordering food more just another meal.
* Elevator: What goes better with Good Fellas than some lasagna and garlic bread?  Got a hankering for some tandoori chicken?  Spice it up with some Slumdog Millionaire.
            We're going to transcend your night on the couch into a night you'll never forget.
* Core Features:  Fetch currently available menus and categories, access netflix for genre search. In the future? Spotify playlists.
...
* Planning:
  1. Problem:  Planning & coordinating a party is hard.  Inviting others, finding food for the party, & seeing who showed up is even more difficult.
  2. Elevator:  Plan your next party using fourinstants.  Setup a Foursquare event, invite friends, & get a list of nearby LS Takeout & Delivery (TOAD) restaurants to cater the event.  See when your friends arrive via Foursquare check-ins.
  3. Core features:  It (1) must be able to scrape LS Takeout & Delivery (TOAD) restaurants; (2) must be able to geocode LS TOAD restaurant addresses; (3) must be able to match Foursquare venues to LS TOAD restaurants; (4) must be able to a user using Foursquare credentials; (5) must be able to pull in a user's Foursquare data.
