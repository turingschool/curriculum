...
The generator has created a Rails application for you. Let's figure out what's in there.

# [Work Marker]

 and automatically open the file `database.yml`.  If you were connecting to an existing database you would enter the database configuration parameters here.  Since we're using SQLite3 and starting from scratch, we can leave the defaults.  Rails will automatically create our database for us.
